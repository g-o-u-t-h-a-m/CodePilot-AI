"""Generic code chunking strategy with fixed-size sliding windows."""
...
class GenericCodeChunkStrategy(ChunkStrategy):
    """Generic chunking strategy for unsupported programming languages.

    Splits code into fixed-size chunks with overlap to preserve context
    across chunk boundaries.
    """

    def __init__(self, chunk_size: int = 120, overlap: int = 20):
        """Initialize the generic code chunking strategy.
    ...
    def chunk(self, source_file: SourceFile) -> List[CodeChunk]:
        ...
        logger.debug(
            f"Created {len(chunks)} generic code chunks for {source_file.relative_path}"
        )
