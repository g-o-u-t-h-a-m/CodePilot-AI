...
class YamlChunkStrategy(ChunkStrategy):
    """Chunking strategy for YAML files.

    Treats the entire YAML file as a single chunk.
    """

    def chunk(self, source_file: SourceFile) -> List[CodeChunk]:
        """Chunk YAML file as a single unit.

        Args:
            source_file: The YAML source file to chunk
        ...
        logger.debug(f"Created single YAML chunk for {source_file.relative_path}")
